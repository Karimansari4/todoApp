...
function UpdateTask({user}) {
    ...
    const getSingleTask = async() => {
        return await axios.get(`http://localhost:5000/task/getSingleTask/${user._id}/${id}`).then((response) => {
            setTask(response.data.result)
        }).catch((err) => {
            console.log("error on getSingleTask: ", err);
        })
    }
    ...
    const handleSubmit = async(evt) => {
        ...
        if(!task.name){
            ...
            return await axios.put(`http://localhost:5000/task/updateTask/${user?._id}/${id}`, task).then((response) => {
    ...
    // console.log("task: ", moment(task.dueDate).format("MM"));
    return (
        <Container>
            ...
            <Typography variant='h4' component={'h6'} mt={2}> <IconButton onClick={() => navigate(-1)}> <ArrowBackIcon /> </IconButton> Update Task</Typography>
            <Box component={'form'} onSubmit={handleSubmit} noValidate sx={{mt: 2}}>
                <Grid container spacing={2}>
                    <Grid item sm={12} xs={12} md={4} lg={3}>
                        <TextField name='name' type='text' label="Task Name" placeholder='Please enter task name' onChange={handleOnChange} value={task.name} fullWidth required error={valTask.name ? valTask.name : '' } helperText={valTask.name ? valTask.name : ''} />
                    </Grid>
                    <Grid item sm={12} xs={12} md={4} lg={3}>
